...
topics: ["go", "golang", "言語仕様", "concurrency", "sync", "atomic"]
published: false
---
...
しかし、その短いコードには、汎用的な実装上の工夫がいくつか詰まっています。
そこで、この記事では、`Once`の内部実装を追いながら、それらのコンセプトを見ていこうと思います。
...
これは、すべての後続処理が安全に実行されるようにするためのようです。特に、`Do`も`f()`も返り値を持たないので、後続処理は`f()`が実行されたかどうかを`Do`の返り値などから直接確かめる方法がありません。そこで、他のプロセスで実行されたいたとしても、`f()`が完了するまでブロックされることで、安全かつ簡潔に後続処理が書けます。
この仕様は、例えば初期化処理に`Once`を利用した際に有用となります。
